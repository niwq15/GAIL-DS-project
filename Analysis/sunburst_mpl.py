...
DATA = {
    ...
    "Data Cleaning &\nPreprocessing": {
        "Data Quality Score": [
            "Normalization\nRange Check", "Data Completeness", "DataFrame Shape\nValidation",
        ],
        "Data Cleaning\nCompleteness": [],
    },
    "Interpre. & RG": [
...
# depth2 = 最外层具体指标圈
LEVEL_RADII = [
    (0.8, 1.55),   # depth 0：大类，内半径0.55(留白)，外半径1.55 —— 宽度=1.0，很宽
    (1.55, 2.05),   # depth 1：子类别，宽度=0.5
    (2.05, 2.45),   # depth 2：具体指标，宽度=0.4
]
...
def draw_node(ax, label, content, depth, theta1, theta2, color, alpha):
    ...
    span = theta2 - theta1
    # 弧长太窄就自动缩小字号，避免文字互相压住
    fontsize = 11 if depth == 0 else max(6, min(9, span / 3))
    wrap_width = 14 if depth == 0 else 10
    wrapped = "\n".join(
